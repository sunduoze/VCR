...
pub fn plot_register_device(device_id: String, channels: Vec<String>) {
    log::info!(
        "[Plot] plot_register_device: device={}, channels={:?}",
        device_id,
        channels
    );
...
pub fn plot_push_data(device_id: String, channel: String, timestamp_ms: f64, value: f64) {
    log::trace!(
        "[Plot] plot_push_data: device={}, ch={}, ts={:.3}, val={:.6}",
        device_id,
        channel,
        timestamp_ms,
        value
...
pub fn plot_push_batch(
    ...
    log::trace!(
        "[Plot] plot_push_batch: device={}, ts={:.3}, chs={:?}, vals={:?}",
        device_id,
        timestamp_ms,
        channels,
        values
...
pub fn plot_push_csv(
    ...
    log::trace!(
        "[Plot] plot_push_csv: device={}, prefix={:?}, ts={:.3}, vals_count={}",
        device_id,
        prefix,
        timestamp_ms,
        values.len()
...
pub fn plot_get_channel_data(device_id: String, channel: String) -> Vec<PlotPoint> {
    ...
    log::debug!(
        "[Plot] plot_get_channel_data: device={}, ch={}, points={}",
        device_id,
        channel,
        result.len()
    );
...
pub fn plot_get_all_channels(device_id: String) -> HashMap<String, Vec<PlotPoint>> {
    ...
    log::debug!(
        "[Plot] plot_get_all_channels: device={}, channels={}",
        device_id,
        result.len()
    );
    result
...
pub fn plot_get_channels(device_id: String) -> Vec<String> {
    ...
    log::debug!(
        "[Plot] plot_get_channels: device={}, channels={:?}",
        device_id,
        result
    );
    result
...
pub fn plot_push_csv_counter(device_id: String, prefix: Option<String>, values: Vec<f64>) {
    ...
    log::trace!(
        "[Plot] plot_push_csv_counter: device={}, prefix={:?}, counter={:.0}, vals_count={}",
        device_id,
        prefix,
        counter,
        values.len()
...
pub fn plot_get_channel_viewport_data(
    ...
    log::debug!(
        "🧪 [DEBUG] [数据链路] 步骤4: Dart 请求数据: device={}, channel={}, 返回 {} 个点",
        device_id,
        channel,
        result.len()
    );
    ...
    let plot_points: Vec<PlotPoint> = result
        .into_iter()
        .map(|p| PlotPoint {
            timestamp_ms: p.timestamp_ms,
            value: p.value,
...
/// 最新数据点（如果没有数据，返回空列表）
#[flutter_rust_bridge::frb(sync)]
pub fn plot_get_channel_latest_data(device_id: String, channel: String) -> Vec<PlotPoint> {
    let result = PLOT_DATA.get_latest_data(&device_id, &channel);
    log::debug!(
        "[Plot] plot_get_channel_latest_data: device={}, ch={}, points={}",
        device_id,
        channel,
        result.len()
    );
