...
class ConsoleDeviceState {
  ...
  void fromJson(Map<String, dynamic> json) {
    ...
    continuousSendInterval = json['continuousSendInterval'] as int? ?? continuousSendInterval;
    continuousSendTarget   = json['continuousSendTarget']   as int? ?? continuousSendTarget;
    commandHistory  = (json['commandHistory'] as List?)?.cast<String>() ?? commandHistory;
  }

  // Reset counters (used after clear or full recalc)
  void resetCounters() {
    txBytes = 0; rxBytes = 0; txPackets = 0; rxPackets = 0; lastLogCount = 0;
    txRate = 0; rxRate = 0; lastTxBytes = 0; lastRxBytes = 0;
  ...
  void updateRates(int newTx, int newRx) {
    ...
    lastTxBytes = newTx; lastRxBytes = newRx; lastRateTime = now;
  }
}
...
class _DebugConsoleScreenState extends State<DebugConsoleScreen> with WidgetsBindingObserver {
  ...
  final TextEditingController _bufferSizeInputController = TextEditingController();
  ...
  ConsoleDeviceState get _cs {
    ...
    return _deviceStates.putIfAbsent(_selectedDeviceId!, () => ConsoleDeviceState());
  }
  ...
  Future<void> _selectDevice(String deviceId, {bool loadFromRust = false}) async {
    // Stop continuous send on current device before switching
    final oldCs = _selectedDeviceId != null ? _deviceStates[_selectedDeviceId!] : null;
    ...
    if (loadFromRust) {
      ...
      cs.isSerialDevice = device != null && device.connectionType == ConnectionType.serial;
  ...
  void _applySignalStates() {
    final cs = _cs;
    if (!cs.connected || !cs.isSerialDevice || _selectedDeviceId == null) return;
  ...
  void _updateCounters(ConsoleDeviceState cs, List<DebugLogEntry> log) {
    ...
    for (final e in log) {
      ...
      if (e.direction == 'TX') { txBytes += bc; txPackets++; }
      else                      { rxBytes += bc; rxPackets++; }
    }
  ...
  void _recalcCounters(ConsoleDeviceState cs, List<DebugLogEntry> log) {
    cs.txBytes = 0; cs.rxBytes = 0; cs.txPackets = 0; cs.rxPackets = 0; cs.lastLogCount = 0;
    for (final e in log) {
      final bc = e.data.length;
      if (e.direction == 'TX') { cs.txBytes += bc; cs.txPackets++; }
      else                     { cs.rxBytes += bc; cs.rxPackets++; }
    }
    cs.lastLogCount = log.length;
    cs.lastTxBytes = cs.txBytes; cs.lastRxBytes = cs.rxBytes;
    cs.lastRateTime = DateTime.now();
    cs.txRate = 0; cs.rxRate = 0;
  }
  ...
  bool _syncDeviceStates() {
    ...
    for (final d in devices) {
      ...
      cs.isSerialDevice = (getDevice(deviceId: d.id)?.connectionType ?? ConnectionType.tcp) == ConnectionType.serial;
  ...
  void _refreshLog() {
    ...
    final cs = _cs;
    final newLog = debugGetLogWithLimit(deviceId: _selectedDeviceId!, maxSize: cs.bufferSize);
    ...
    if (cs.autoScroll && _scrollController.hasClients) {
      ...
      Future.delayed(const Duration(milliseconds: 150), () { _isProgrammaticScroll = false; });
    }
  }
  ...
  String _buildLogText(List<DebugLogEntry> log) {
    ...
    return filtered.map((entry) {
      final parts = <String>[];
      if (_cs.showTimestamp) parts.add('[${_formatTimestamp(entry.timestamp)}]');
      parts.add('[${entry.direction}]');
      parts.add(_cs.showHex
        ? entry.data.map((b) => b.toRadixString(16).toUpperCase().padLeft(2,'0')).join(' ')
        : entry.display);
      return parts.join(' ');
    }).join('\n');
  }
  ...
  Future<void> _exportLog() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Exported to $outputPath'), duration: const Duration(seconds: 2)),
        );
      }
  ...
  int? _parseBufferSize(String input) {
    ...
    if (mb != null) {
      ...
      if (v != null) { final b = (v * 1024 * 1024).round(); if (b >= 1 && b <= 500*1024*1024) return b; }
    }
    final kb = RegExp(r'^(\d+)\s*KB$').firstMatch(input);
    if (kb != null) {
      final v = int.tryParse(kb.group(1)!);
      if (v != null) { final b = v * 1024; if (b >= 1 && b <= 500*1024*1024) return b; }
    }
    return null;
  }
        ...
        content: Column(
          ...
          children: [
            TextField(
              ...
              decoration: const InputDecoration(hintText: 'e.g. 200 KB or 1.5 MB', border: OutlineInputBorder(), suffixText: 'max: 500 MB'),
              onSubmitted: (v) {
                final sz = _parseBufferSize(v);
                if (sz != null) { _applyBufferSize(sz); Navigator.of(ctx).pop(); }
                else { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Invalid format'))); }
              },
            ),
            ...
            Wrap(
              ...
              children: _bufferSizePresets.map((sz) => ActionChip(
                label: Text(_formatBufferSizeInput(sz), style: const TextStyle(fontSize: 12)),
                onPressed: () { _bufferSizeInputController.text = _formatBufferSizeInput(sz); },
              )).toList(),
            ),
          ],
        ),
  ...
  void _sanitizeHexInput() {
    ...
    if (s != text) {
      ...
      _inputController.selection = TextSelection.fromPosition(TextPosition(offset: s.length));
    }
  }
  ...
  List<int>? _encodeInput() {
    ...
    if (_cs.sendFormat == 'ASCII') {
      ...
      String full = text;
      if (_cs.lineEnding == 'CR')      full = '$text\r';
      else if (_cs.lineEnding == 'LF') full = '$text\n';
      else if (_cs.lineEnding == 'CRLF') full = '$text\r\n';
      ...
      if (_cs.lineEnding == 'CR')      bytes.add(0x0D);
      else if (_cs.lineEnding == 'LF') bytes.add(0x0A);
      else if (_cs.lineEnding == 'CRLF') { bytes.add(0x0D); bytes.add(0x0A); }
      return bytes;
    }
    return null;
  }
  ...
  void _doSend() {
    ...
    final text = _inputController.text;
    if (_cs.sendFormat == 'ASCII') _addToHistory(text);
    else if (_cs.sendFormat == 'HEX') _addToHistory('[HEX] $text');
  ...
  void _showContinuousSendDialog() {
    ...
    final iCtrl = TextEditingController(text: _cs.continuousSendInterval.toString());
    final cCtrl = TextEditingController(text: _cs.continuousSendTarget.toString());
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(controller: iCtrl, decoration: const InputDecoration(labelText: 'Interval (ms)', border: OutlineInputBorder(), suffixText: 'ms'), keyboardType: TextInputType.number),
            const SizedBox(height: 12),
            TextField(controller: cCtrl, decoration: const InputDecoration(labelText: 'Count', border: OutlineInputBorder()), keyboardType: TextInputType.number),
          ],
        ),
        actions: [
          TextButton(onPressed: () => Navigator.of(ctx).pop(), child: const Text('Cancel')),
  ...
  Future<void> _continuousSendLoop(ConsoleDeviceState cs) async {
    ...
    while (cs.continuousSending && !cs.continuousStopRequested && cs.continuousSendBytes != null && targetDeviceId != null) {
      ...
      if (cs.continuousSendCount % 10 == 0 || cs.continuousSendCount >= cs.continuousSendTarget) {
        if (mounted) setState(() {});
      }
      if (cs.continuousSendCount >= cs.continuousSendTarget) break;
  ...
  Future<void> _sendFile() async {
    ...
    try {
      ...
      if (!await file.exists()) { _showError('File not found: $path'); return; }
      final bytes = await file.readAsBytes();
      if (bytes.isEmpty) { _showError('File is empty'); return; }
      _addToHistory('[FILE] $path (${bytes.length} bytes)');
      debugSendBytes(deviceId: _selectedDeviceId!, data: bytes);
      _refreshLog();
    } catch (e) { _showError('Failed to read file: $e'); }
  }
  ...
  void _startPolling() { _stopPolling(); _pollTimer = Timer.periodic(const Duration(milliseconds: kPollIntervalMs), (_) => _refreshLog()); }
  ...
  Color _getDirectionColor(String dir) => dir == 'TX' ? Colors.orange : Colors.green;
  ...
  String _formatBytes(int bytes) {
    ...
    if (bytes < 1024*1024) { final kb = bytes/1024; return '${kb.toStringAsFixed(kb==kb.roundToDouble()?0:1)} KB'; }
    final mb = bytes/(1024*1024); return '${mb.toStringAsFixed(mb==mb.roundToDouble()?0:1)} MB';
  }
  ...
  String _formatBufferSize(int bytes) => bytes < 1024*1024 ? '${bytes~/1024} KB' : '${bytes~/(1024*1024)} MB';
  ...
  Widget _buildToggleIcon({required IconData icon, required bool active, required String tooltip, required VoidCallback onTap}) {
    return Tooltip(message: tooltip, preferBelow: false, child: GestureDetector(
      ...
      child: Container(
        ...
        child: Icon(icon, size: 18, color: active ? AppTheme.textPrimary : AppTheme.textSecondary.withValues(alpha: 0.4)),
      ),
    ));
  }

  Widget _buildDirectionToggle({required String label, required bool active, required Color activeColor, required String tooltip, required VoidCallback onTap}) {
    return Tooltip(message: tooltip, preferBelow: false, child: GestureDetector(
      onTap: onTap,
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(4),
          border: active ? Border.all(color: activeColor.withValues(alpha: 0.5), width: 1) : null,
        ),
        child: Text(label, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, fontFamily: 'Consolas, monospace', color: active ? activeColor : AppTheme.textSecondary.withValues(alpha: 0.35))),
      ),
    ));
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_selectedDeviceId != null && _connected) {
      rawLog = debugGetLogWithLimit(deviceId: _selectedDeviceId!, maxSize: _cs.bufferSize);
    }
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          Tooltip(message: 'Set buffer size (up to 500 MB)', preferBelow: false, child: TextButton.icon(
            onPressed: _showBufferSizeDialog,
            icon: const Icon(Icons.storage, size: 18),
            label: Text(_formatBufferSize(_cs.bufferSize), style: const TextStyle(fontSize: 12)),
          )),
          IconButton(icon: const Icon(Icons.save_alt), onPressed: filteredLog.isNotEmpty ? _exportLog : null, tooltip: 'Export Log'),
          IconButton(icon: const Icon(Icons.copy), onPressed: filteredLog.isNotEmpty ? _copyLog : null, tooltip: 'Copy Log'),
          IconButton(icon: const Icon(Icons.delete_outline), onPressed: filteredLog.isNotEmpty ? _clearLog : null, tooltip: 'Clear Log'),
        ],
      ),
      body: Column(children: [
            ...
            Expanded(
              child: DropdownButtonFormField<String>(
                ...
                items: devices.map((d) {
                  ...
                  final isConnected = dCs?.connected ?? (d.status == DeviceStatus.connected);
                  return DropdownMenuItem(
                  value: d.id,
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    Container(width: 8, height: 8, decoration: BoxDecoration(shape: BoxShape.circle, color: isConnected ? AppTheme.success : AppTheme.error)),
                    const SizedBox(width: 8),
                    Flexible(child: Text(d.name, overflow: TextOverflow.ellipsis, softWrap: false)),
                  ]),
                );}).toList(),
                onChanged: (v) {
                  if (v != null) { _selectDevice(v).then((_) { if (mounted) setState(() {}); }); }
                },
              ),
            ),
            const SizedBox(width: 12),
            Tooltip(message: _connected ? 'Disconnect' : 'Connect', preferBelow: false, child: GestureDetector(
              onTap: _selectedDeviceId == null ? null : _toggleConnection,
              child: Container(
                padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                decoration: BoxDecoration(color: _connected ? AppTheme.success : AppTheme.error, borderRadius: BorderRadius.circular(6)),
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Icon(_connected ? Icons.link_off : Icons.link, size: 18, color: Colors.white),
                  const SizedBox(width: 6),
                  Text(_connected ? 'Disconnect' : 'Connect', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                ]),
              ),
            )),
          ]),
        ...
        Container(
          ...
          child: Row(children: [
            _buildToggleIcon(icon: Icons.schedule,     active: _cs.showTimestamp, tooltip: 'Show Timestamp', onTap: () { _cs.showTimestamp = !_cs.showTimestamp; _saveSignalStates(); setState(() {}); }),
            const SizedBox(width: 4),
            _buildToggleIcon(icon: Icons.data_object,  active: _cs.showHex,        tooltip: 'Show as HEX',     onTap: () { _cs.showHex        = !_cs.showHex;        _saveSignalStates(); setState(() {}); }),
            const SizedBox(width: 4),
            _buildToggleIcon(icon: Icons.vertical_align_bottom, active: _cs.autoScroll, tooltip: 'Auto Scroll', onTap: () { _cs.autoScroll = !_cs.autoScroll; _saveSignalStates(); setState(() {}); }),
            const SizedBox(width: 12),
            _buildDirectionToggle(label: 'Tx', active: _cs.showTx, activeColor: Colors.orange, tooltip: 'Show Tx', onTap: () { _cs.showTx = !_cs.showTx; _saveSignalStates(); setState(() {}); }),
            const SizedBox(width: 4),
            _buildDirectionToggle(label: 'Rx', active: _cs.showRx, activeColor: Colors.green,  tooltip: 'Show Rx', onTap: () { _cs.showRx = !_cs.showRx; _saveSignalStates(); setState(() {}); }),
            const Spacer(),
            if (_cs.continuousSending)
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                decoration: BoxDecoration(color: Colors.blue.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.blue.withValues(alpha: 0.4))),
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  const SizedBox(width: 12, height: 12, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.blue)),
                  const SizedBox(width: 6),
                  Text('${_cs.continuousSendCount}/${_cs.continuousSendTarget}', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold, fontFamily: 'Consolas, monospace', color: Colors.blue)),
                ]),
              ),
            if (_cs.continuousSending) const SizedBox(width: 12),
            Tooltip(message: 'Tx: ${_cs.txPackets} pkts, ${_formatBytes(_cs.txBytes)}', preferBelow: false, child: Row(mainAxisSize: MainAxisSize.min, children: [
              Text('Tx', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, fontFamily: 'Consolas, monospace', color: Colors.orange.withValues(alpha: 0.8))),
              const SizedBox(width: 4),
              Text(_formatBytes(_cs.txBytes), style: TextStyle(fontSize: 11, fontFamily: 'Consolas, monospace', color: AppTheme.textSecondary)),
              const SizedBox(width: 4),
              Text(_formatRate(_cs.txRate),  style: TextStyle(fontSize: 10, fontFamily: 'Consolas, monospace', color: AppTheme.textSecondary.withValues(alpha: 0.7))),
            ])),
            const SizedBox(width: 16),
            Tooltip(message: 'Rx: ${_cs.rxPackets} pkts, ${_formatBytes(_cs.rxBytes)}', preferBelow: false, child: Row(mainAxisSize: MainAxisSize.min, children: [
              Text('Rx', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, fontFamily: 'Consolas, monospace', color: Colors.green.withValues(alpha: 0.8))),
              const SizedBox(width: 4),
              Text(_formatBytes(_cs.rxBytes), style: TextStyle(fontSize: 11, fontFamily: 'Consolas, monospace', color: AppTheme.textSecondary)),
              const SizedBox(width: 4),
              Text(_formatRate(_cs.rxRate),  style: TextStyle(fontSize: 10, fontFamily: 'Consolas, monospace', color: AppTheme.textSecondary.withValues(alpha: 0.7))),
            ])),
          ]),
        ),
          ...
          child: Container(
            ...
            child: filteredLog.isEmpty
                ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    Text('No data yet', style: TextStyle(color: Colors.grey[600], fontSize: 16)),
                    const SizedBox(height: 8),
                    Text('Connect a device and send data', style: TextStyle(color: Colors.grey[700], fontSize: 12)),
                  ]))
                : NotificationListener<UserScrollNotification>(
                    onNotification: (n) {
                      if (n.direction != ScrollDirection.idle && _cs.autoScroll) {
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return Padding(
                              ...
                              child: RichText(
                                text: TextSpan(
                                  style: const TextStyle(fontFamily: 'Consolas, monospace', fontSize: 13, color: Colors.white),
                                  children: [
                                    if (_cs.showTimestamp) TextSpan(text: '[${_formatTimestamp(entry.timestamp)}] ', style: TextStyle(color: Colors.grey[500])),
                                    TextSpan(text: '[${entry.direction}] ', style: TextStyle(color: _getDirectionColor(entry.direction), fontWeight: FontWeight.bold)),
          ...
          child: Row(children: [
            ...
            if (_cs.sendFormat == 'ASCII')
              SizedBox(
                ...
                child: DropdownButtonFormField<String>(
                  ...
                  onChanged: (v) { _cs.encoding = v!; _saveSignalStates(); setState(() {}); },
                ),
              ),
            ...
            SizedBox(
              ...
              child: DropdownButtonFormField<String>(
                ...
                onChanged: (v) { _cs.lineEnding = v!; _saveSignalStates(); setState(() {}); },
              ),
            ),
            ...
            if (_cs.commandHistory.isNotEmpty)
              PopupMenuButton<String>(
                ...
                onSelected: (cmd) { _inputController.text = cmd; _inputFocusNode.requestFocus(); },
                itemBuilder: (context) => _cs.commandHistory.asMap().entries.map((e) => PopupMenuItem<String>(
                  value: e.value,
                  height: 36,
                  child: Row(children: [
                    SizedBox(width: 24, child: Text('${e.key+1}', style: TextStyle(color: AppTheme.textSecondary, fontSize: 11))),
                    const SizedBox(width: 8),
                    Flexible(child: Text(e.value, style: const TextStyle(fontFamily: 'monospace', fontSize: 13), maxLines: 1, overflow: TextOverflow.ellipsis)),
                  ]),
                )).toList(),
              ),
            ...
            Expanded(
              child: TextField(
                ...
                inputFormatters: _cs.sendFormat == 'HEX' ? [_hexFormatter] : null,
                decoration: InputDecoration(
                  hintText: _cs.sendFormat == 'HEX' ? 'e.g. AA BB CC or AABBCC' : 'Enter command or data...',
            ...
            if (_cs.continuousSending)
              ElevatedButton.icon(
                ...
                label: Text('Stop (${_cs.continuousSendCount}/${_cs.continuousSendTarget})'),
                style: ElevatedButton.styleFrom(backgroundColor: Colors.red, foregroundColor: Colors.white),
              )
            else
              Row(mainAxisSize: MainAxisSize.min, children: [
                ElevatedButton.icon(
                  onPressed: _connected && _selectedDeviceId != null ? _send : null,
                  icon: const Icon(Icons.send, size: 18),
                  label: const Text('Send'),
                  style: ElevatedButton.styleFrom(backgroundColor: AppTheme.primary),
                ),
                const SizedBox(width: 4),
                Tooltip(message: 'Continuous Send', preferBelow: false, child: ElevatedButton(
                  onPressed: _connected && _selectedDeviceId != null ? _showContinuousSendDialog : null,
                  style: ElevatedButton.styleFrom(backgroundColor: Colors.blue.shade700, padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 0), minimumSize: const Size(36, 36)),
                  child: const Icon(Icons.repeat, size: 18, color: Colors.white),
                )),
              ]),
          ]),
        ),
    ...
    return Padding(
      ...
      child: Tooltip(
        ...
        child: OutlinedButton(
          onPressed: _connected && _selectedDeviceId != null ? () => _sendQuickCommand(cmd) : null,
