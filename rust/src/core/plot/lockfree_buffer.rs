...
///
/// Layout:
/// [write_pos: AtomicU64] [padding] [read_pos: AtomicU64] [padding] [capacity: usize]
...
impl LockFreeRingBuffer {
    ...
    pub fn push(&self, timestamp_ms: f64, value: f64) {
        ...
        // Update readable count (monotonically increasing, up to capacity)
        let new_readable = (idx + 1).min((self.read_pos.load(Ordering::Acquire) as usize) + self.capacity);
        self.readable.store(new_readable as u64 - self.read_pos.load(Ordering::Acquire), Ordering::Release);
    }
    ...
    pub fn push_batch(&self, points: &[(f64, f64)]) {
        ...
        let readable = (new_idx).min(
            (self.read_pos.load(Ordering::Acquire) as usize) + self.capacity
        );
        self.readable.store(
